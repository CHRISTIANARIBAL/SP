...
#     goods = Goods.objects.all().order_by('-date_added')
#     if query:
#         goods = goods.filter(
...
#         good_instance = get_object_or_404(Goods, id=good_id)
...
#     categories = Category.objects.all()
#     customers = Customer.objects.all()  # Fetch all customers for selection

#     if request.method == 'POST':
#         # Retrieve the barcode and other form data
...
#         customer_id = request.POST.get('customer')  # Retrieve the selected customer ID

#         # Get the corresponding Category and Customer
#         category = Category.objects.get(id=category_id)
#         customer = Customer.objects.get(id=customer_id)

#         # Create the new product and associate it with the customer
...
#             category=category,
#             customer=customer  # Associate the product with the selected customer
#         )

#         return redirect('dashboard')  # Redirect to dashboard or product listing page
...
#         if barcode:
#             try:
#                 product = Goods.objects.get(barcode=barcode)
